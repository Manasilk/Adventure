...
execute unless entity @e[type=#server:creature,tag=REQ_QUEST_TARGET,distance=0..31.999] run tag @s add _e.get_spawn
execute if score @s[tag=_e.get_spawn] entity_upd_t = #dbc.server cfg.quest_t run function server:scripts/quests/entity_spawn/get_spawn_list
