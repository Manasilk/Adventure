...
execute if score #dbc.server tick_upd_t matches 0 as @e[type=#server:area_trigger,tag=!ENTITY_FLAG_QUEST_TRIGGER] at @s run function server:scripts/world/area_trigger_script_loader
execute if score #dbc.server tick_upd_t matches 0.. as @e[type=#server:area_trigger,tag=ENTITY_FLAG_QUEST_TRIGGER] at @s run function server:scripts/world/area_trigger_script_loader
