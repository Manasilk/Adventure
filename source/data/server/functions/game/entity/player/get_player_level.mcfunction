...
execute if score @s entity_reclvl > @s entity_maxlvl run tag @s add _e.error_level_invalid
tag @s[tag=_e.error_level_invalid] remove _e.player_levelup_msg
#Send a message, playsound and display "happy" particles only to the invoker
playsound minecraft:ui.player.levelup master @s[tag=_e.player_levelup_msg] ~ ~ ~ 0.3 1 0.1
