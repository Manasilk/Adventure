...
tag @a[tag=_h.search_break,scores={_h.interact=1..},distance=0..3.999] remove _h.search_break
#Tell the player they can't accept any more quests if the quest log is full
execute if entity @s[team=QuestStart] run tag @a[scores={_h.interact=1..,quest_log=10},distance=0..3.999] add _e.quest_log_full_msg
...
execute if entity @s[team=QuestEnd] if entity @a[scores={_h.interact=1..,quest_status=1..},distance=0..3.999] run function server:game/entity/conversation/send_quest_complete
execute if entity @s[team=QuestStart] if entity @a[tag=!_h.gossip_break,scores={_h.interact=1..,quest_log=0..9},distance=0..3.999] run function server:game/entity/conversation/send_quest_to_invoker
