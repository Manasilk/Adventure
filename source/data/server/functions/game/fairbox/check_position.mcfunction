...
execute unless block ~ ~0.55 ~ #server:ignore run teleport @s[gamemode=!spectator] ~ ~1 ~
...
execute if score @s _FB._T.Update matches 0.. run function server:game/fairbox/metrics/get_player_position_new
...
execute as @e[type=minecraft:player,tag=FB_FLAG_PLAYER_IGNORE,tag=SPELL_AURA_GHOST,scores={_FB._T.Update=5}] run function server:game/fairbox/events/remove_flag_ignore_player
execute as @e[type=minecraft:player,tag=rbac.commands,tag=SPELL_AURA_GHOST,scores={_FB._T.Update=-1}] run function server:game/fairbox/events/remove_flag_ignore_player
