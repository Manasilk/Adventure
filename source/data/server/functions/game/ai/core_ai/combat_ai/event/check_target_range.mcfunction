execute store success score @s[tag=TARGET_PLAYER_MELEE] has_target if entity @a[tag=los_target,tag=ENTITY_FLAG_SPELL_TARGET,distance=0..1.199,sort=nearest,limit=1]
...
execute if score @s[tag=SMARTCAST_DYNAMIC_TARGET] has_target = #bool false run function server:game/ai/core_ai/combat_ai/event/switch_target