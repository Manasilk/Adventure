execute if entity @s[type=#server:creature] run summon minecraft:area_effect_cloud ~ ~1 ~ {CustomName:"{\"text\":\"VMAP_LOS_CHK\"}",CustomNameVisible:0,Duration:5,Tags:["vmap_los_chk","ENTITY_FLAG_WORLD_IGNORE","SUMMONER_CREATURE"]}
execute if entity @s[type=minecraft:player] run summon minecraft:area_effect_cloud ~ ~1 ~ {CustomName:"{\"text\":\"VMAP_LOS_CHK\"}",CustomNameVisible:0,Duration:5,Tags:["vmap_los_chk","ENTITY_FLAG_WORLD_IGNORE","SUMMONER_PLAYER"]}
