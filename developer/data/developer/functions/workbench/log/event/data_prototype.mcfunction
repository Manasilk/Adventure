#TEMPORARY, WIP
execute store result storage internal:debug log.event[-1].data[].player.id int 1 run scoreboard players get @a[limit=1] player_id
execute store result storage internal:debug log.event[-1].data[].player.gamemode int 1 run scoreboard players get @a[limit=1] player_gamemode
...
data modify storage internal:debug log.event[-1].data[].player.position append from entity @e[type=minecraft:area_effect_cloud,sort=nearest,limit=1] FallDistance
